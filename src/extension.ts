...
const registerCommands = (ctx: vscode.ExtensionContext): void => {
  ...
  ctx.subscriptions.push(
    ...
    cmd('foxtrot.runActiveFuzzer', async () => {
      ...
      if (!(st.activePlugin && st.activePart && st.activeFuzzer)) {
        vscode.window.showWarningMessage('Select plugin, part and fuzzer first (Bit-Gen sidebar).');
        return;
      }
...
export const activate = async (ctx: vscode.ExtensionContext): Promise<void> => {
  ...
  /* ---------- Phase & bootstrap ---------- */
  void switchPhase.switchPhase('bitgen');
